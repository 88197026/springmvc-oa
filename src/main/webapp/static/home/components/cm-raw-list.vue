<template>
    ...
    <table class="table table-hover table-th">
        <thead>
				  <tr>
							<th>预览图</th>
						  <th>模板名称</th>
        ...
        <tbody>
				   <tr v-for="(index, item) in items">
						 <td><img v-bind:src="item.previewUrl" width="200" /></td>
		         <td>{{ item.name }}</td>
						 <td>{{ item.width }}*{{ item.height }}</td>
		         <td>{{item.info}}</td>
		         <td>{{item.createTime}}</td>
						 <td>{{item.createTime}}</td>
		         <td>{{ item.createNick }}</td>
		         <td>{{ item.id }}</td>
             <td v-if="item.isLock==0">
    ...
    import modalManage from '../components/modal-manage.vue';
        ...
        ready() {
            var self = this;
						...
						dispatch.on("xj-createMode",function(obj){
							self.$refs.modalmanage.showCreateUpPsdModal();
						})
        },
        methods:{
